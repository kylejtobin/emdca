...
**Scope of Work:**
1.  **Migrate Patterns to Rules**: Convert `spec/patterns/*.md` into individual `.cursor/rules/` directories. Each rule will contain the pattern logic and examples, configured to apply intelligently based on file context (e.g., "Aggregate" rules applying to `entity.py` files).
2.  **Refactor Skeleton Strategy**: Transform the `spec/src/` "docstring skeletons" into a mechanism that reinforces the new Rules. This may involve converting them into Rule templates or updating their references to point to the new active Rules, ensuring a file creation event triggers the correct architectural context.
3.  **Restructure Documentation**: Move high-level documentation (`arch.md`, `agentic.md`) to `docs/` and clear out the `spec/` directory to eliminate ambiguity between "documentation for humans" and "rules for agents."
